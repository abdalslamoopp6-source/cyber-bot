...
# =========================
# إعدادات البوت والخدمات (التوكن الجديد)
# =========================

TOKEN = '8506228695:AAF3oceD6X7eLM_ED6eNAKe4DhDfyTeyCrs'
...
# =========================
# حماية المغادرة التلقائية للمجموعات (منع استغلال البوت في المجموعات)
# =========================
...
async def send_main_menu(update: Update, context: ContextTypes.DEFAULT_TYPE, is_callback=False):
    ...
    conn = sqlite3.connect('users.db')
    cursor = conn.cursor()
    cursor.execute('SELECT user_id, coins FROM users WHERE user_id = ?', (user.id,))
    row = cursor.fetchone()
    if row is None:
        cursor.execute(
    ...
    keyboard = [
        ...
        [
            InlineKeyboardButton('🟢 استرجاع رقم', callback_data='service_recover_whatsapp'),
        ...
        [
            ...
            InlineKeyboardButton('💬 شحن كوينز / التواصل', url=f'https://t.me/{TARGET_USER.replace("@", "")}'),
        ],
    ]
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # التأكد أن الأمر يتم في المحادثة الخاصة فقط
...
async def charge_user(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user = update.effective_user
    if not user.username or user.username.lower() != TARGET_USER.replace('@', '').lower():
        return
    ...
    if row is None:
        ...
        cursor.execute('UPDATE users SET coins = ? WHERE user_id = ?', (new_coins, target_id))
    conn.commit()
    conn.close()

    await update.message.reply_text(f'✅ تم إضافة {amount} كوينز للمستخدم `{target_id}` بنجاح.', parse_mode='Markdown')
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == 'my_account':
        ...
        conn = sqlite3.connect('users.db')
        cursor = conn.cursor()
        cursor.execute('SELECT coins, joined_at FROM users WHERE user_id = ?', (user.id,))
        row = cursor.fetchone()
        conn.close()
        ...
        keyboard = [
            [InlineKeyboardButton('💬 التواصل مع المالك', url=f'https://t.me/{TARGET_USER.replace("@", "")}')],
        ...
        user = query.from_user
        if user.username and user.username.lower() == TARGET_USER.replace('@', '').lower():
            conn = sqlite3.connect('users.db')
            cursor = conn.cursor()
            cursor.execute('SELECT COUNT(*) FROM users')
            total_users = cursor.fetchone()[0]
            conn.close()
            text = f'📊 **لوحة الإحصائيات (خاص بالمسؤول):**\n\n👥 إجمالي المستخدمين: `{total_users}`'
        else:
