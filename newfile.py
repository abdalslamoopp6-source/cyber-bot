...
async def receive_target_data(
    ...
    user = update.effective_user
    s_name = context.user_data.get('pending_service', 'خدمة عامة')

    conn = sqlite3.connect('users.db')
    cursor = conn.cursor()
    cursor.execute(
        'INSERT OR REPLACE INTO pending_requests (user_id, service_name, target_input) VALUES (?, ?, ?)',
    ...
    user_msg = (
        '✅ تم إرسال الطلب للمالك بنجاح.\n\n'
        '🔴 سيتم مراجعة الطلب وإرسال السعر لك.\n'
        '⚠️ الخدمة ليست مجانية وسعرها بالكوينز.\n'
        '💳 الكوينز تقدر تشحنها من المالك من الزر الموجود في الشاشة الرئيسية.'
    )
    keyboard = [
...
# ==========================
# مغادرة المجموعات والقنوات تلقائياً
# ==========================

async def auto_leave_groups(update: Update, context: ContextTypes.DEFAULT_TYPE):
...
def main():
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            ...
            WAITING_FOR_TARGET_INPUT: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, receive_target_data)
            ],
            WAITING_FOR_ACCOUNT_FOLLOWERS: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, receive_account_followers)
            ],
            WAITING_FOR_ACCOUNT_USERNAME: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, receive_account_username)
            ],
        },
    ...
    application.add_handler(CommandHandler('price', admin_set_price))
    application.add_handler(ChatMemberHandler(auto_leave_groups, ChatMemberHandler.MY_CHAT_MEMBER))
